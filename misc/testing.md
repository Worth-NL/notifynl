...
{% hint style="info" %}
All testing takes place in the production environment. There is no test environment for NotifyNL.
{% endhint %}
...
You can use these smoke test numbers and addresses with any [type of API key](api-keys.md).

You can smoke test all NotifyNL API client functions except:
...
You must use a [test API key](api-keys.md#test) to do non-smoke testing such as performance or integration testing. You can use any non-smoke testing phone numbers or email addresses. You do not need a specific Notify testing account.
