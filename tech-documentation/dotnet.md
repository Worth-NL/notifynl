---
description: NotifyNL supports .NET framework 4.6.2 and .NET Core 2.0.
icon: microsoft
---
...
TheNotify client can be installed from [Nuget.org](https://www.nuget.org/packages/GovukNotify/).

You can install the Notify client package using either the command line or Microsoft Visual Studio.
...
var client = new NotificationClient("https://api.notifynl.nl", apiKey);
```
...
Make sure you do not exceed our [rate limits](https://docs.notifications.service.gov.uk/net.html#rate-limits).
...
{% include "../.gitbook/includes/if-you-use-the-test-api-key....md" %}

#### **Error codes**

If the request is not successful, the client returns a `Notify.Exceptions.NotifyClientException` containing the relevant error code. To learn more about error structure, go to [Errors section](dotnet.md#errors).
...
Find out how to [reduce the risk of malicious content injection in placeholders](dotnet.md#reducing-the-risk-of-malicious-content-injection-in-placeholders).
...
3. Call the [sendEmail method](dotnet.md#send-an-email).
...
* check the response to the [original notification method call](https://docs.notifications.service.gov.uk/net.html#response)
...
* [email status descriptions](dotnet.md#email-status-descriptions)
* [text message status descriptions](dotnet.md#text-message-status-descriptions)
* [letter status descriptions](dotnet.md#letter-status-descriptions)
* [precompiled letter status descriptions](dotnet.md#precompiled-letter-status-descriptions)
...
This API call returns the data for multiple messages. You can get either the most recent messages, or get older messages by specifying a particular notification ID in the `olderThanId` argument.
...
* [email status](dotnet.md#email-status-descriptions)
* [text message status](dotnet.md#text-message-status-descriptions)
* [letter status](dotnet.md#letter-status-descriptions)
* [precompiled letter status](dotnet.md#precompiled-letter-status-descriptions)
...
* check the response to the [original notification method call](dotnet.md#get-the-data-for-one-message)
* [sign in to NotifyNL](https://admin.notifynl.nl/sign-in) and go to the **API integration** page
...
* [various schema validation errors](dotnet.md#schema-validation-errors), for example when you forget to pass in an argument, or pass in an argument of a wrong type.
* errors that are not related to generating a preview template, but instead are related to things like authentication and rate limits. You can find a list of these errors [in General errors](dotnet.md#general-errors)

## Errors

### Error handling

If the request is not successful, the client will throw a `Notify.Exceptions.NotifyClientException`.

The exceptions message attribute is a string consisting of:
...
You can use a regex match to get specific information out of the exception’s message attribute. An illustration of error handling:
...
{
    string pattern = """(?<=Status code )([0-9]+)""";
    Regex r = new Regex(pattern, RegexOptions.IgnoreCase);
    Match match = r.Match(ex.Message);
    int statusCode = Int32.Parse(match.Value);
    if (statusCode >= 500)
    {
        //some retry logic goes here
    }
    else
    {
...
The exception’s message attribute contains a `string`, for example:
...
* [send a message](dotnet.md#send-a-message)
* [get message data](dotnet.md#get-message-data)
* [get a template](dotnet.md#get-a-template)
* [get received text messages](dotnet.md#get-received-text-messages)
